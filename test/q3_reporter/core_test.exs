defmodule Q3Reporter.CoreTest do
  ...
  describe "Core.interpret_log/2" do
    ...
    test "with default mode" do
      assert %Results{entries: [%{}], mode: :by_game} = Core.interpret_log(@content, mode: :by_game)
    end

    test "with ranking mode" do
      assert %Results{entries: [%{}], mode: :ranking} = Core.interpret_log(@content, mode: :ranking)
    end
  end
end
